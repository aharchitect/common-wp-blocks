...
test( 'create and render admonition block', async ( { page, baseURL } ) => {
    ...
    const resolvedBase = baseURL || process.env.WP_BASE_URL || 'http://localhost:8000';
    ...
    console.log('Login successful, opening new post editor...');
    ...
    try {
        await page.waitForSelector( '.editor-post-title__input, textarea.editor-post-title__input, .edit-post-visual-editor__block-list, .block-editor-writing-flow', { timeout: 45000 } );
        console.log('Editor loaded!');
    } catch ( e ) {
        // Print debug info before failing or retrying
        const url = page.url();
        let content = '';
        try { content = await page.content(); } catch (err) { content = '[could not get page content]'; }
        console.log('Editor did not load. Current URL:', url);
        console.log('Page content snippet:', content.slice(0, 20000));
    ...
    const writingFlow = page.locator( '.block-editor-writing-flow, .edit-post-visual-editor__block-list' ).first();
    if ( await writingFlow.count() ) {
        ...
        const titleInput = page.locator( '.editor-post-title__input, textarea.editor-post-title__input' ).first();
    ...
    if ( ! inserted ) {
        // Open block inserter
        const inserterToggle = page.locator( 'button[aria-label="Add block"], button.editor-inserter__toggle' ).first();
        if ( await inserterToggle.count() ) {
            ...
            if ( await searchInput.count() ) {
                ...
                const admonionButton = page.locator( 'button:has-text("Admonition"), div[role="button"]:has-text("Admonition")' ).first();
    ...
    const contentArea = page.locator( '.admonition-content p, .admonition-content' ).first();
    // If there's an inner paragraph, click it; otherwise click the content container and type
    if ( await contentArea.count() ) {
        await contentArea.click();
        await page.keyboard.type( 'This is an admonition created by E2E test.' );
    } else {
    ...
    try {
        await page.click( 'button.editor-post-publish-button__button, button[aria-label="Publish"]', { timeout: 5000 } );
    } catch ( e ) {
        // Try alternative selector
        await page.click( 'button.editor-post-publish-panel__toggle, .editor-post-publish-button', { timeout: 5000 } );
    }

    // Confirm publish if confirmation shown
    try {
        await page.waitForSelector( 'button.editor-post-publish-panel__header-publish-button, button.editor-post-publish-button__button', { timeout: 3000 } );
        await page.click( 'button.editor-post-publish-panel__header-publish-button, button.editor-post-publish-button__button' );
    } catch ( e ) {
        // Likely single click publish completed
    }

    // Wait for post published notice and click View Post
    await page.waitForSelector( 'a.editor-post-publish-panel__postpublish-buttons__view-link, a.post-publish-panel__postpublish-buttons a', { timeout: 10000 } );
    const viewLink = page.locator( 'a.editor-post-publish-panel__postpublish-buttons__view-link, a.post-publish-panel__postpublish-buttons a' ).first();
    const href = await viewLink.getAttribute( 'href' );
    if ( href ) {
        // If href is relative, make absolute
        const target = href.startsWith( 'http' ) ? href : `${ resolvedBase }${ href }`;
    ...
    await page.waitForSelector( '.admonition-title, .admonition-content', { timeout: 10000 } );
    await expect( page.locator( '.admonition-title' ) ).toContainText( 'E2E Note Title' );
    await expect( page.locator( '.admonition-content' ) ).toContainText( 'This is an admonition created by E2E test.' );
    // Check base class for default type
    await expect( page.locator( 'details.admonition-type-note, .admonition-type-note' ) ).toHaveCount( 1 );
} );
