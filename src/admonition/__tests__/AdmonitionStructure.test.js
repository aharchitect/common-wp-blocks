...
jest.mock( '@wordpress/block-editor', () => {
	...
	MockRichText.Content = ( { value, tagName: Tag, className } ) => (
		<Tag data-testid="rich-text-save" className={ className }>
			{ value }
		</Tag>
	);
	...
	const MockInnerBlocks = ( props ) => (
		<div
			data-testid="inner-blocks-edit"
			data-props={ JSON.stringify( props ) }
		/>
	);
	MockInnerBlocks.Content = () => <div data-testid="inner-blocks-save" />;
...
describe( 'AdmonitionStructure', () => {
	...
	it( 'should render in SAVE mode with correct components and attributes', () => {
		...
		expect(
			screen.queryByTestId( 'inner-blocks-edit' )
		).not.toBeInTheDocument();
	...
	it( 'should set the <details open> attribute based on isOpen prop', () => {
		...
		const { rerender } = render( <AdmonitionStructure { ...props } /> );
